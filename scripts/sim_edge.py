#!/usr/bin/env python3
"""Simulate a real ANDON node from a laptop without a Pico.
...
request and answers with a sealed andon command. Students can validate
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the ANDON edge simulator"
    ) from exc
...
def _tick(ser, args, seq):
    """Transmit one simulated sealed ANDON zone request.
