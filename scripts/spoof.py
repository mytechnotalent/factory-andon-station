#!/usr/bin/env python3
"""Inject a forged ANDON ZONE into a node to teach authentication.
...
command deployed the diverter; against the hardened node the same attack fails
...
try:
    ...
    raise SystemExit(
        "pip install pyserial to run the ANDON spoof tool"
    ) from exc
...
def _add_forge_args(parser):
    ...
    parser.add_argument("--zone", type=int, default=DEFAULT_ZONE,
                        help="Forged line zone identifier")
...
def main():
    """Inject the forged ANDON ZONE under the gateway address.
