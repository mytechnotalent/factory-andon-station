 ...
 *
 * DESCRIPTION:
 * Native unit tests for the RP2350 IRON CHOIR factory andon station
 * tower light, fault-clear button, line diverter servo, infrared decoder,
...
static void assert_lamps(int red, int yellow, int green) {
    ...
    TEST_ASSERT_EQUAL_INT(yellow,
                          mock_gpio_get(ANDON_YELLOW_LED_PIN));
    TEST_ASSERT_EQUAL_INT(green,
                          mock_gpio_get(ANDON_GREEN_LED_PIN));
}
 ...
 * @return void
 */
static void assert_show(andon_led_state_t state, int red, int yellow,
                        int green) {
...
static void test_status_led_show(void) {
    ...
    assert_show(ANDON_OFF, 0, 0, 0);
    assert_show(ANDON_FAULT, 1, 0, 0);
    assert_show(ANDON_ACK_PENDING, 0, 1, 0);
    assert_show(ANDON_LINE_OK, 0, 0, 1);
}
 ...
 * @return void
 */
static void test_clear_pressed(void) {
    reset_pc();
    TEST_ASSERT_TRUE(clear_init());
    gpio_put(ANDON_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(clear_pressed());
    gpio_put(ANDON_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(clear_pressed());
}
 ...
 * @return void
 */
static void test_clear_consume(void) {
    reset_pc();
    clear_init();
    gpio_put(ANDON_BUTTON_PIN, true);
    TEST_ASSERT_FALSE(clear_consume_press());
    gpio_put(ANDON_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(clear_consume_press());
    TEST_ASSERT_FALSE(clear_consume_press());
}
 ...
 * @return void
 */
static void clear_release(void) {
    gpio_put(ANDON_BUTTON_PIN, true);
    clear_consume_press();
}
 ...
 * @return void
 */
static void test_clear_debounce(void) {
    reset_pc();
    clear_init();
    gpio_put(ANDON_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(clear_consume_press());
 ...
 * @return void
 */
static void test_clear_debounce_elapsed(void) {
    reset_pc();
    clear_init();
    gpio_put(ANDON_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(clear_consume_press());
    clear_release();
    mock_timer_set_us(mock_timer_now_us() + ANDON_CLEAR_DEBOUNCE_US);
    gpio_put(ANDON_BUTTON_PIN, false);
    TEST_ASSERT_TRUE(clear_consume_press());
}

/**
 * @brief Verify clear_reset clears the debounce state.
 *
 * @param void No parameters.
 * @return void
 */
...
static void test_servo_init(void) {
    ...
    slice = pwm_gpio_to_slice_num(ANDON_SERVO_PIN);
...
static void test_ir_init(void) {
    ...
    TEST_ASSERT_FALSE(s_mock_gpio_dirs[ANDON_IR_PIN]);
}
...
static void run_indicator_tests(void) {
    ...
    RUN_TEST(test_clear_pressed);
    RUN_TEST(test_clear_consume);
    RUN_TEST(test_clear_debounce);
    RUN_TEST(test_clear_debounce_elapsed);
    RUN_TEST(test_clear_reset);
}
